...
## Phone Auth & Backend Impact

The Phone Number + OTP login feature is implemented entirely on the client-side using Firebase Auth and Firestore. It does not introduce any new Cloud Functions.

### Backend Assumptions

Users authenticated via Phone Auth are subject to the same backend rules and assumptions as email-authenticated users:

1. **User Profile**: Every user must have a corresponding document in the `users/{uid}` collection in the `elajtech` database.
2. **User Type**: The `userType` field (admin, doctor, or patient) must be correctly set for role-based Cloud Function logic (e.g., appointment management).
3. **Active Status**: Users must pass `isActive` checks before accessing role-restricted data or APIs.
