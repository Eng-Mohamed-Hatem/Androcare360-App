...
### 10. Phone Number Format Rule (E.164)

- All phone numbers **must** use E.164 format (start with `+` followed by digits only, no spaces or dashes).
- UI forms must validate against this format (e.g. using `r'^\+\d{8,15}$'`).
- Firestore fields (e.g. `users.phoneNumber`) must always store the normalized E.164 value.
- Do not store or compare raw local formats such as `0100XXXXXXX`; always convert and use the E.164 representation instead.
...
### 11. Authentication & Routing Change Policy

Any change that touches authentication flows, user identity, or role-based routing must:
- Preserve all existing authentication-related tests (no regressions are allowed in the current 700+ test suite).
- Add or update unit and/or widget tests for every new scenario (e.g., new roles, new account states, new routing branches).
- Ensure `databaseId: elajtech` is used for all Firestore access in auth-related repositories, and that missing or malformed documents are handled gracefully without crashes.
...
Phone Number + OTP login is a critical authentication flow. All contributions touching the `auth` feature must ensure:

1. **AuthRepositoryImpl Unit Tests**: Cover `verifyPhoneNumber`, `signInWithPhoneNumber`, and error cases (invalid OTP, network failure).
2. **AuthProvider/AuthNotifier State Tests**: Verify loading states, transition to OTP screen, successful authentication, and `isActive` check logic.
3. **(Optional) Flow Verification**: Run end-to-end integration tests to ensure the sequence from login screen to role-specific dashboard is intact.
